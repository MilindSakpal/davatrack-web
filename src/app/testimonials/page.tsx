              ...
              <button
                ...
                className={cn(
                  ...
                  activeSegment === seg
                    ...
                    : "bg-white text-[#122631] hover:bg-[#EEF4F3] border border-[#CAD7D0] shadow-sm"
                )}
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {filtered.map((item, idx) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`rounded-3xl p-8 sm:p-10 transition-all duration-300 flex flex-col justify-between relative group overflow-hidden ${
                    isDark
                ...
                >
                  ...
                  <Quote className={`absolute top-6 right-8 w-16 h-16 pointer-events-none ${
                    isDark ? "text-white/15" : "text-white/30"
                  }`} />

                  <div className="relative z-10 space-y-5">
                    <div className="flex items-center justify-between">
                      <span className={`px-3 py-1 rounded-full text-xs font-mono uppercase tracking-wider font-bold shadow-xs ${
                        isDark
                          ? "bg-white/15 text-white border border-white/25 backdrop-blur-md"
                          : "bg-[#122631] text-white"
                      }`}>
                        {item.segment}
                      </span>
                      <span className={`text-[11px] font-mono font-bold px-2.5 py-1 rounded-full shadow-xs ${
                        isDark
                          ? "text-white bg-white/10 border border-white/20"
                          : "text-[#122631] bg-white/80 border border-white/70"
                      }`}>
                        Case Highlight
                      </span>
                    </div>

                    <blockquote className={`text-base sm:text-lg font-bold leading-relaxed ${
                    ...
                    {item.metrics && (
                      <div className="grid grid-cols-2 gap-3 pt-3">
                        {item.metrics.map((m, mIdx) => (
                          <div key={mIdx} className={`p-3 rounded-xl shadow-xs backdrop-blur-sm ${
                            isDark
                  ...
                  <div className={`pt-6 mt-6 border-t flex items-center justify-between relative z-10 ${
                    ...
                    <div>
                      ...
                      <div className={`text-xs font-medium ${
                        isDark ? "text-white/80" : "text-[#122631]/80"
                      }`}>
                    ...
                    <div className={`text-xs font-mono font-bold uppercase tracking-wider ${
                      isDark ? "text-[#6EBCBF]" : "text-[#122631]"
                    }`}>
                      VERIFIED PARTNER
                    </div>
                  </div>
